...
fn gz_tar_check_file() {
    ...
    match archive::read::contains_file("tests/test_archives/a-file.tar.gz", "tar_gz_a_file") {
        Ok(result) => assert!(result),
        Err(_) => assert!(false),
    }
    match archive::read::contains_file("tests/test_archives/1-folder-nest.tar.gz", "tar_gz_1_folder_a_file") {
        Ok(result) => assert!(result),
        Err(_) => assert!(false),
    }
    match archive::read::contains_file("tests/test_archives/2-folders-nest.tar.gz", "tar_gz_2_folder_1_folder_a_file") {
        Ok(result) => assert!(result),
        Err(_) => assert!(false),
    }
    match archive::read::contains_file("tests/test_archives/2-folders-nest.tar.gz", "a_files") {
